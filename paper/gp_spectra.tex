...
\begin{abstract}
Upcoming million-star spectroscopic surveys have the potential to revolutionize our view of the formation and chemical evolution of the Milky Way. Realizing this potential requires automated approaches to optimise estimates of stellar properties, %to optimise estimates of stellar properties 
such as chemical element abundances, from the spectra. The sheer volume and quality of the observations strongly motivate that these approaches should be driven by the data. With this in mind, we introduce SSSpaNG: a data-driven Non-Gaussian Process %mixture 
model of Stellar Spectra. We implement SSSpaNG using APOGEE red clump stars, whose modeling parameters we infer using Gibbs sampling. By pooling information between stars to infer their covariance, we permit clear identification of the correlations between spectral pixels. Harnessing this correlation structure, we infer and reconstruct a higher fidelity spectrum for each red clump star, inpainting missing %spectral 
regions and de-noising by a factor of at least 2-3, for low-signal-to-noise stars. These high-fidelity inferred spectra, produced with our approach, will enable improved chemical elemental abundance measurements, for individual stars. %Modeling the spectra as a Gaussian Process 
...
Our Gaussian Process model goes beyond standard approaches. As we sample the individual elements of the signal covariance matrix, the prior for the true spectra is very non-Gaussian. Were we to fit the covariance once and hold it fixed, as is common in the field, the true spectra would be Gaussian-distributed. By marginalizing over the covariance, these distributions become very heavy-tailed, promoting sparse (i.e., typically small but occasionally large) excursions from the mean. As a result, we name the code SSSpaNG: Stellar Spectra as Sparse, data-driven, Non-Gaussian processes.

\smf{To demonstrate the effectively non-Gaussian nature of the signal prior we can explicitly marginalize over the true signal covariance $\speccov$. Considering only the terms that depend on $\speccov$ for one mixture component gives
\begin{align}
\prob\left(\objspec_i|\specmean,\objclass_i\right) & = 
\int
	\prob\left(\objspec_i|\specmean_k,\speccov_k,\objclass_i\right)
	\prob\left(\speccov_k\right)
d\speccov_k \nonumber \\
& \propto \int
	\left| \speccov_{\objclass_i} \right|^{-\frac{\left( \nb+1 \right)}{2}}
	e^{-\frac{1}{2} \left[\left(\objspec_i-\specmean_{\objclass_i} \right) \speccov^{-1} \left(\objspec_i-\specmean_{\objclass_i} \right)^T\right]}
d\speccov_{\objclass_i} \nonumber \\
& \propto 
e^{-\frac{\nb}{2}\ln{\left|\left(\objspec_i-\specmean_{\objclass_i} \right) \otimes \left(\objspec_i-\specmean_{\objclass_i} \right) \right|}}.
\end{align}
This is a non-convex highly sparsifying prior. Conceptually, it strongly prefers spectra close to the class mean, but if a spectrum differs greatly from the mean it is only penalized logarithmically. }\bdw{This is a bit abbreviated. The statement is precise for a single spectrum; for  more spectra it's true if the deviations from the mean are orthogonal. It's easier to see if the eigenbasis of S are fixed a priori and there is a single spectrum, then the statement above is true for the expansion coefficients of the spectrum in spectrum in the eigenbasis.}
