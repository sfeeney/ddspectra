...
The network diagram for our hierarchical Bayesian model is shown in Fig.~\ref{fig:network_diagram}, with the parameters, data and constants described in Table~\ref{tab:params} and the probability distributions defining each link set out in Table.~\ref{tab:prob_dists}. The particular set of probability distributions chosen allow for the conditional distributions of each latent variable to be written analytically, and we hence use Gibbs sampling~\cite{Geman_and_Geman:1984} to estimate the joint posterior. \textcolor{red}{mkn: sf can you expand on Gibbs sampling a bit and also explain this network diagram pedagogically for the uninitiated? I want to keep our audience on top of all the specialised language summarised in these figures, many readers will have never seen before coming from the spectral world.}

\begin{figure}
	\includegraphics[width=\columnwidth]{bhm_plot.pdf}
    \caption{Network diagram for our hierarchical Bayesian model which is a graphical representation of our implemented modeling of the data. See Table 1 for the parameter descriptions. \smf{include prob dists in population frames.}}
