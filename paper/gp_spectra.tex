...
\begin{table}
    \centering
    \caption{Parameters used in our probabilistic model.}
    \label{tab:params}
    \begin{tabular}{ll}
        \hline
        parameter & description \\
        \hline
        $\specmean$ & $\uniform\left(-\infty,\infty\right)$ \\
        $\speccov$ & $\left| \speccov \right|^{-\left(\nb+1\right)/2}$ \\
        $\alpha$ & $\dirichlet\left(\alpha\right)$ \\
        $\classprobs$ & $\dirichlet\left(\alpha\right)$ \\
        $\objspec_i$  \\
        $\objclass_i$ \\
        $\objdata_i$ & $\normal\left(\objspec_i,\objnoise_i\right)$ \\
        $\objnoise_i$ & $\normal\left(\objspec_i,\objnoise_i\right)$ \\
        \hline
    \end{tabular}
\end{table}
...
\begin{figure}
    ...
    \caption{This Figure shows the validation of the model and the method showing the recovery of an artificially masked portion of the spectra. This region corresponds to a Cerium line centered around 15789 \AA\ for a star with a SNR of 23 \textbf{check this SNR}, which demonstrates the performance of the model even for low SNR data. The inferred spectrum is shown in the dark-grey shaded region and the measured spectra is shown in the solid black line. The model is in excellent agreement with the data once noise is taken into account (light-grey shaded region).}
...
\begin{figure*}
    ...
    \caption{Information gains for our C, Na, Mg, Fe, Ce and Ge windows given observations of other elemental windows. Each point on the x-axis states the most informative window remaining after conditioning on all previous elemental windows. Thus, one would learn most about the C window (top left) by observing Y, then adding Ce, Cr, {\it et cetera}. Elements belonging to illustrative families are indicated by coloured labels, with the predicted element's family members picked out in bold.}
...
\begin{figure*}
    ...
    \caption{Conditional standard deviations for our C, Na, Mg, Fe, Ce and Ge windows given observations of other elemental windows, added in the order set out in the corresponding panel of Fig.~\ref{fig:single_element_information}. The uncertainty on the predicted spectrum without observations of other windows is shown in black. \bdw{Is this the same as the noise?} \smf{No.} }
...
\begin{figure}
    ...
    \caption{Information gains for all pairs of elemental windows, with elements grouped by similar information gains, using five-\AA ngstr\"om-wide windows.}
