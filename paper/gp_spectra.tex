...
\begin{enumerate}
\item Predict masked (unmeasured or contaminated) regions of the spectra to make, e.g., abundance measurements that would otherwise be impossible. This may be particularly valuable in APOGEE for neutron capture element measurements. For these elements in particular,  only one or two features exist from which to make measurements. Valuable elements like Nd and Ce add the additional dimensionality of the neutron-capture nucleosynthetic family (see Section~\ref{sec:data}). Some of these element features may also fall near the one of three chip gaps and may be missing in spectra and not others due to stellar velocities. Our modeling of the data can predict these regions when they are absent. 
\item Examine in detail the empirical correlations in the spectra, make quantitative measurements of these correlations and identify in practice which elements are positively and which are negatively affiliated (see Section~\ref{sec:results}). %[Section 2: might just want to add a plot of which are + and which are - correlated] 
\item De-noise to make precision measurements at lower SNR.  Generically, we can use the data-driven Gaussian Process mixture model to de-noise and thus enhance the data, potentially enabling new discoveries. This should work for high SNR and low SNR regions; how useful this is depends on the size of the effects we may want to discover (see Section~\ref{sec:inference}). Our expectation is that this is particularly good for very weak features where our expectation is that our generated spectra can do a lot better with abundance measurements, similarly to that previously demonstrated with generative modeling (e.g. with The Cannon and The Payne).
...
\begin{enumerate}
\item DONE: restate motivation
\item DONE: proved this concept using reduced dataset. shown that we can capture correlations between spectral pixels using data-driven model. shown that this model allows us to inpaint reliably. shown that we can denoise. yields better spectra (see mkn's first point). push down required snr. don't worry about masking as much.
\item DONE: furthermore, we have demonstrated how to quantify the information gained by observing portions of spectra and used this to define the most mutually informative regions of spectra. great for designing future experiments to permit inference of unobserved elements.
\item critical to address the current limitations
\item currently restricted to 25 narrow windows for comp reasons. results clearly show adding more elements is better (even for the short list we have). limited by matrix inversion. two ways to address: low-rank or brute force (1 inversion per sample per star plus one more?) moore's law?
\item should data volume preclude full analysis, would need to restrict to windows as we've done here. any results would depend entirely on the windows selected, so list should be optimised. we've not done so here, just picked strongest per element, so our results are indicative. while we have demonstrated here the method's ability to discover weak lines, restricting to windows clearly reduces the volume in which to make such serendipitous discoveries, a cost that needs to be factored in to the decision to restrict to windows. `` We hesitate to make strong statements at this point about the information content of individual elements, however, as our results are sensitive to the bandwidth of the window taken around each element absorption feature.''
\item conclusions drawn from this analysis are bayesian: depend on model! thus, if model isn't good fit, bad conclusions. currently restricted to single class: i.e., assume red clump is multivariate gaussian in spectral space. not necessarily true, want to open up multiple classes. improve sampler. will allow fitting of single non-gaussian pops, multiple pops, outliers, etc. currently outliers will (if few) be dragged toward main pop or (if many) increase covariance to stretch single mvn over two populations.
\end{enumerate}

In this work, we have demonstrated how to pool information from ensembles of stellar spectra in order to denoise and inpaint individual observations, with the aim of optimizing the quality and quantity of deliverables such as stellar abundances and ages from upcoming million-star surveys. We do so by modeling the distribution of 29502 APOGEE red clump stars' binned spectra as a high-dimensional Gaussian Process whose covariance matrix describes the variations in spectra within the population. Inferring the elements of this covariance matrix directly, we have shown that this completely data-driven model is capable of capturing the correlations between spectral pixels and harnessing them to yield improved estimates of individual spectra, along with precise {\it and} accurate predictions for unobserved spectral pixels. By producing complete spectra with decreased uncertainties for each member of the population (reducing flux errors by a factor of 2-3 for stars with SNR $\approx$ 20), we enable improved abundances to be made for all elements for every star, thereby relaxing the data quality assumed necessary for precision abundance estimation. We have demonstrated our method's potential using the recently discovered 15789 \AA\ cerium line, a high-value APOGEE target due to its s-process provenance~\citep{Cunha2017}. Our model makes accurate and differentiable predictions for the cerium line in low-SNR stars in which this line is completely masked, permitting confident measurements of cerium abundance where previously impossible.
...
MORE HERE: critical to address the current limitations!
...
%\mkn{ choices - (i) we didn't optimise over any lines, we selected nominally around the strongest lines - (ii) discovery space }
